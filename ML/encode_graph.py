...
def one_hot_encoding(x, permitted_list):
    """Maps input elements x which are not in the permitted list to the last
    element of the permitted list."""
...
def get_atom_features(atom, use_chirality=True, hydrogens_implicit=True):
    """Takes an RDKit atom object as input and gives a 1d-numpy array of atom
    features as output."""
    ...
    formal_charge_enc = one_hot_encoding(
        int(atom.GetFormalCharge()), [-3, -2, -1, 0, 1, 2, 3, 4, 5, 6, 7, 8, "Extreme"]
    )
...
def get_bond_features(bond, use_stereochemistry=False):
    """Takes an RDKit bond object as input and gives a 1d-numpy array of bond
    features as output."""
...
def create_pytorch_geometric_graph_data_list_from_smiles_and_labels(
    x_smiles, y, ids, use_chirality=True
):
    """
    Inputs:
    ...
    for smiles, y_val, id_val in zip(x_smiles, y, ids):
        # print(smiles)
        ...
        n_node_features = len(
            get_atom_features(
                unrelated_mol.GetAtomWithIdx(0), use_chirality=use_chirality
            )
        )
        n_edge_features = len(
        ...
        for atom in mol.GetAtoms():
            X[atom.GetIdx(), :] = get_atom_features(atom, use_chirality=use_chirality)
        ...
        data_list.append(
            Data(x=X, edge_index=E, edge_attr=EF, y=y_tensor, identifier=id_val)
        )
